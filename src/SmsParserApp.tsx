...
{/* <ScrollView style={styles.container}> */}
      // <Text style={styles.title}>Categorized SMS Messages</Text>
      {/* <FlatList
        data={Object.keys(categorizedMessages)}
        keyExtractor={(item, index) => index.toString()}
        renderItem={({ item }) => (
          <View style={styles.category}>
            <Text style={styles.categoryTitle}>{item.toUpperCase()}</Text>
            <FlatList
              data={categorizedMessages[item]}
              keyExtractor={(message, index) => index.toString()}
              renderItem={({ item: message }) => (
                <View style={styles.message}>
                  <Text style={styles.sender}>Sender: {message.address}</Text>
      ...
      {/* {Object.keys(categorizedMessages).map((category) => (
        ...
        {/* Second FlatList for Payments */}
    {/* {categorizedMessages['payments'] && categorizedMessages['payments'].length > 0 && (
      <View style={styles.category}>
        <Text style={styles.categoryTitle}>Payments</Text>
        <FlatList
          data={categorizedMessages['payments']}
          showsVerticalScrollIndicator = {false}
          keyExtractor={(message, index) => index.toString()}
          renderItem={({ item: message }) => {
            const paymentDetails = parsePaymentMessage(message.body);
            ...
            return (
              <View style={styles.paymentCard}>
                ...
                {paymentDetails.type === 'debit' && paymentDetails.to && (
                  <>
                    <Text style={styles.details}>To: {paymentDetails.to}</Text>
                    <Text style={styles.details}>Amount: {paymentDetails.amount}</Text>
                    <Text style={styles.details}>Ref. No: {paymentDetails.refNo}</Text>
                ...
                {paymentDetails.type === 'credit' && paymentDetails.from && (
                  <>
                    <Text style={styles.details}>From: {paymentDetails.from}</Text>
                    <Text style={styles.details}>Amount: {paymentDetails.amount}</Text>
                    <Text style={styles.details}>Ref. No: {paymentDetails.refNo}</Text>
